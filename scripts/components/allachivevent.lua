...
function allachivevent:seffc(inst, tag)
    ...
    if _G.NOTIFICATION then
        TheNet:Announce(inst:GetDisplayName() .. "   " .. strinfo .. STRINGS.GUI["space"] .. STRINGS.GUI["complA"] ..
                            strname .. STRINGS.GUI["br2"])
    end
    inst.components.talker:Say(STRINGS.GUI["br1"] .. strname .. STRINGS.GUI["br2"] .. "\n" .. STRINGS.GUI["obt"] ..
                                   allachiv_coinget[tag] .. STRINGS.GUI["points"])
...
function allachivevent:eatfn(inst)
    inst:ListenForEvent("oneat", function(inst, data)
        if inst:GetDisplayName() ~= data.feeder:GetDisplayName() and data.feeder.components.allachivevent.feedplayer ~=
            true and findprefab(foodmenu, data.food.prefab) then
        ...
        -- Warm Up
        if inst.components.temperature.current <= 0 and inst.components.health.currenthealth > 0 and
            findprefab(heatfood, food.prefab) and self.eathot ~= true and self.eattemp ~= true then
        ...
        -- Cool Down
        if inst.components.temperature.current >= 70 and inst.components.health.currenthealth > 0 and
            findprefab(coldfood, food.prefab) and self.eatcold ~= true and self.eattemp ~= true then
    ...
    inst:DoPeriodicTask(5, function()
        ...
        if self.saddle ~= true then
            local item = inst.components.inventory:FindItem(function(item)
                return item.prefab == "saddle_race"
            end)
            if (item ~= nil) then
                self.saddles = 1
                self.saddle = true
        ...
        if self.banana ~= true then
            local items = inst.components.inventory:FindItems(function(item)
                return item.prefab == "cave_banana_cooked"
            end)
            local count = 0
            for i, item in ipairs(items) do
                count = count + (item.components.stackable ~= nil and item.components.stackable:StackSize() or 1)
        ...
        if self.spore ~= true then
            local items = inst.components.inventory:FindItems(function(item)
                return item.prefab == "spore_small"
            end)
            local countgreen = 0
            for i, item in ipairs(items) do
            ...
            items = inst.components.inventory:FindItems(function(item)
            ...
            if (countgreen >= allachiv_eventdata["spore"] and countred >= allachiv_eventdata["spore"] and countblue >=
                allachiv_eventdata["spore"]) then
                self.spore = true
        ...
        if self.blueprint ~= true then
            local items = inst.components.inventory:FindItems(function(item)
                return string.match(item.prefab, "blueprint?") == "blueprint"
            end)
            local count = 0
            for i, item in ipairs(items) do
                count = count + (item.components.stackable ~= nil and item.components.stackable:StackSize() or 1)
        ...
        if self.boat ~= true then
            local itemWheel = inst.components.inventory:FindItem(function(item)
                return item.prefab == "steeringwheel_item"
            end)
            local itemMast = inst.components.inventory:FindItem(function(item)
                return item.prefab == "mast_item"
            end)
            local itemAnchor = inst.components.inventory:FindItem(function(item)
                return item.prefab == "anchor_item"
            end)
            local itemOar = inst.components.inventory:FindItem(function(item)
                return item.prefab == "oar"
            end)
            local itemBoat = inst.components.inventory:FindItem(function(item)
                return item.prefab == "boat_item"
            end)
            local count = 0
            if itemWheel ~= nil then
                count = count + 1
            end
            if itemMast ~= nil then
                count = count + 1
            end
        ...
        if self.moonrock ~= true then
            local items = inst.components.inventory:FindItems(function(item)
                return item.prefab == "moonrocknugget"
            end)
            local count = 0
            for i, item in ipairs(items) do
                count = count + (item.components.stackable ~= nil and item.components.stackable:StackSize() or 1)
        ...
        if self.gnome ~= true then
            local items = inst.components.inventory:FindItems(function(item)
                return item.prefab == "trinket_4" or item.prefab == "trinket_13"
            end)
            local count = 0
            for i, item in ipairs(items) do
                count = count + (item.components.stackable ~= nil and item.components.stackable:StackSize() or 1)
...
function allachivevent:onwalkfn(inst)
    inst:DoPeriodicTask(1, function()
        ...
        if inst.components.locomotor.wantstomoveforward then
            -- Walk or Ride
            if inst.components.rider ~= nil and inst.components.rider:IsRiding() and inst.components.rider.mount ~= nil and
                inst.components.rider.mount.prefab == "beefalo" then
...
function allachivevent:onkilled(inst)
    inst:ListenForEvent("death", function(inst, data)
        ...
        if attacker and attacker.prefab and attacker:IsValid() and self.tooyoung ~= true then
            if attacker.prefab == "flint" or attacker.prefab == "rocks" or attacker.prefab == "redgem" or
                attacker.prefab == "bluegem" or attacker.prefab == "goldnugget" or attacker.prefab == "nitre" or
                attacker.prefab == "marble" then
                inst:DoTaskInTime(2, function()
                    self.tooyoung = true
...
function allachivevent:sanitycheck(inst)
    inst:DoPeriodicTask(1, function()
        ...
        if inst.components.sanity.current >= inst.components.sanity.max * 0.95 and self.fullsanity ~= true and
            inst.components.health.currenthealth > 0 then
...
function allachivevent:hungercheck(inst)
    inst:DoPeriodicTask(1, function()
        if inst.components.hunger.current >= inst.components.hunger.max * 0.95 and self.fullhunger ~= true and
            inst.components.health.currenthealth > 0 then
...
function allachivevent:onkilledother(inst)
    inst:ListenForEvent("killed", function(inst, data)
        ...
        -- Fugu Hutch
        if victim and victim.prefab == "hutch" and victim.components.amorphous:GetCurrentForm() == "FUGU" and self.hutch ~=
            true and inst.components.health.currenthealth <= 10 then
            self.hutch = true
        ...
        if victim and victim.prefab == "tentacle_pillar" and self.hentai ~= true then
            ...
            for k, v in pairs(ents) do
                if (v:HasTag("player") or v.prefab == "bunnyman" or v.prefab == "hutch" or v.prefab == "rocky" or
                    v.prefab == "pigman") and v ~= inst then
                    single = false
                end
            end
            if single == true then
        ...
        -- werepig
        if victim and
            (victim.prefab == "moonpig" or (victim.prefab == "pigman" and victim.components.werebeast:IsInWereState())) then
            local pos = Vector3(victim.Transform:GetWorldPosition())
            local ents = TheSim:FindEntities(pos.x, pos.y, pos.z, 30)
            for k, v in pairs(ents) do
                if v:HasTag("player") then
        ...
        if victim and victim.prefab == "krampus" then
            ...
            inst:DoTaskInTime(.1, function()
                ...
                for k, v in pairs(ents) do
                    if v.prefab == "krampus_sack" and v.components.inventoryitem.owner == nil and
                        v.components.ksmark.mark == false then
        ...
        if victim and victim.prefab == "klaus" then
            ...
            inst:DoTaskInTime(1, function()
                ...
                for k, v in pairs(ents) do
                    if v.prefab == "klaussackkey" and v.components.inventoryitem.owner == nil and
                        v.components.ksmark.mark == false then
                        v.components.ksmark.mark = true
        ...
        -- Shadow Chess
        if victim and
            (victim.prefab == "shadow_knight" or victim.prefab == "shadow_rook" or victim.prefab == "shadow_bishop") and
            victim.level == 3 then
            local pos = Vector3(victim.Transform:GetWorldPosition())
            local ents = TheSim:FindEntities(pos.x, pos.y, pos.z, 30)
            inst:DoTaskInTime(2, function()
        ...
        if victim and victim.prefab == "deerclops" then
            ...
            for k, v in pairs(ents) do
                if v:HasTag("player") then
                    ...
                    if v.components.allachivevent.bossautumn and v.components.allachivevent.bossantlion and
                        v.components.allachivevent.bossspring and v.components.allachivevent.bosswinter and
                        v.components.allachivevent.king ~= true then
...
function allachivevent:makefriend(inst)
    function inst.components.leader:AddFollower(follower)
        if self.followers[follower] == nil and follower.components.follower ~= nil then
            ...
            -- Spooders
            if (follower.prefab == "spider" or follower.prefab == "spider_dropper" or follower.prefab ==
                "spider_warrior" or follower.prefab == "spider_hider" or follower.prefab == "spider_spitter") and
                achiv.spooder ~= true then
            ...
            -- Mandrake
            if follower.prefab == "mandrake_active" and achiv.evil ~= true and
            ...
            follower:PushEvent("startfollowing", {
                leader = self.inst
            })
...
function allachivevent:miner(inst)
    inst:ListenForEvent("finishedwork", function(inst, data)
        if data.target and
            (data.target:HasTag("boulder") or data.target:HasTag("statue") or findprefab(rocklist, data.target.prefab)) then
...
function allachivevent:respawn(inst)
    inst:ListenForEvent("respawnfromghost", function(inst, data)
        ...
        if data and data.source and data.source.prefab == "amulet" and table.getn(_G.AllPlayers) == 1 and self.messiah ~=
            true then
...
function allachivevent:ontimepass(inst)
    inst:DoPeriodicTask(5, function(inst)
        ...
        if self.lavae ~= true then
            local tooth = inst.components.inventory:FindItem(function(item)
                return item.prefab == "lavae_tooth"
            end)
        ...
        if self.musichutch ~= true then
            local bowl = inst.components.inventory:FindItem(function(item)
                return item.prefab == "hutch_fishbowl"
            end)
            local hutch = nil
            if bowl then
        ...
        if self.shadowchester ~= true then
            local eyebone = inst.components.inventory:FindItem(function(item)
                return item.prefab == "chester_eyebone"
            end)
            local chester = nil
            if eyebone then
...
function allachivevent:onplant(inst)
    inst:ListenForEvent("deployitem", function(inst, data)
        if (data.prefab == "pinecone" or data.prefab == "twiggy_nut" or data.prefab == "dug_berrybush" or data.prefab ==
            "dug_berrybush_jucy" or data.prefab == "dug_grass" or data.prefab == "dug_marsh_bush" or data.prefab ==
            "dug_sapling" or data.prefab == "dug_berrybush2" or data.prefab == "dug_rock_avocado_bush" or data.prefab ==
            "dug_sapling_moon" or data.prefab == "acorn") and self.nature ~= true then
...
function allachivevent:ontemperature(inst)
    inst:DoPeriodicTask(1, function()
        if inst.components.temperature.current <= 0 and self.icebody ~= true and inst.components.health.currenthealth >
            0 then
    ...
    inst:DoPeriodicTask(1, function()
        if inst.components.temperature.current >= 70 and self.firebody ~= true and inst.components.health.currenthealth >
            0 then
...
function allachivevent:onhunger(inst)
    inst:DoPeriodicTask(1, function()
        if inst.components.hunger.current <= 0 and self.starve ~= true and inst.components.health.currenthealth > 0 then
...
function allachivevent:onlearn(inst)
    inst:ListenForEvent("learnrecipe", function(inst, data)
        ...
        if blueprint_str == "ruinsrelic_table" or blueprint_str == "ruinsrelic_chair" or blueprint_str ==
            "ruinsrelic_vase" or blueprint_str == "ruinsrelic_plate" or blueprint_str == "ruinsrelic_bowl" or
...
function allachivevent:allget(inst)
    if self.all ~= true then
        inst:DoPeriodicTask(1, function()
            if self.all ~= true then
                if self.intogame and self.firsteat and self.supereat and self.danding and self.messiah and self.emerald and
                    self.citrin and self.amber and self.saddle and self.banana and self.spore and self.blueprint and
                    self.boat and self.moonrock and self.gnome and self.mosquito and self.walkalot and self.stopalot and
                    self.tooyoung and self.evil and self.snake and self.deathalot and self.nosanity and self.sick and
                    self.coldblood and self.burn and self.freeze and self.goodman and self.brother and self.fishmaster and
                    self.pickmaster and self.pickappren and self.chopmaster and self.chopappren and self.noob and
                    self.cookmaster and self.cookappren and self.longage and self.luck and self.black and
                    self.buildmaster and self.buildappren and self.minemaster and self.mineappren and self.tank and
                    self.angry and self.icebody and self.firebody and self.rigid and self.ancient and self.queen and
                    self.moistbody and self.king and self.santa and self.knight and self.bishop and self.rook and
                    self.starve and self.catperson and self.rose and self.butcher and self.goatperd and self.mossling and
                    self.weetusk and self.secondchance and self.nature and self.alldiet and self.allgiantPlants and
                    self.hentai and self.hutch and self.spooder and self.minotaur and self.dragonfly and self.malbatross and
                    self.crabking and self.sleep and self.trader and self.fuzzy and self.pet and self.caveage and
                    self.birdclop and self.eathot and self.eatcold and self.rot and self.knowledge and self.dance and
                    self.oldage and self.superstar and self.rocklob and self.teleport and self.eatfish and
                    self.eatturkey and self.eatpepper and self.eatbacon and self.eatmole and self.sleeptent and
                    self.sleepsiesta and self.reviveamulet and self.feedplayer and self.bathbomb and self.shadowchester and
                    self.snowchester and self.musichutch and self.lavae and self.evilflower and self.roses and
                    self.drown and self.dmgnodmg and self.bullkelp and self.horrorhound and self.slurtle and
                    self.werepig and self.fruitdragon and self.treeguard and self.spiderqueen and self.varg and
                    self.koaelefant and self.monkey and self.lightning and self.birchnut and self.rider and
                    self.fullsanity and self.fullhunger and self.pacifist then
                    self.all = true
                        ...
                        inst:DoTaskInTime(.3, function()
                            ...
                            for i = 1, 25 do
                                inst:DoTaskInTime(i / 25 * 3, function()
                                    local pos = Vector3(inst.Transform:GetWorldPosition())
                                    SpawnPrefab("explode_firecrackers").Transform:SetPosition(
                                        pos.x + math.random(-3, 3), pos.y, pos.z + math.random(-3, 3))
                                end)
                            end
                        end)
