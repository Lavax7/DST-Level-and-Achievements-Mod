...
return Class(function(self, inst)
    ...
    local function StartHunt()
        ...
        inst:DoTaskInTime(0, StartCooldown, newhunt,
            TUNING.HUNT_COOLDOWN + TUNING.HUNT_COOLDOWNDEVIATION * (math.random() * 2 - 1))
        return newhunt
    end
    ...
    local function IsEligible(player)
        ...
        return
            TheWorld.Map:IsVisualGroundAtPoint(player.Transform:GetWorldPosition()) and area:GetCurrentArea() ~= nil and
                not area:CurrentlyInTag("nohunt") and not area:CurrentlyInTag("moonhunt")
    end
    ...
    OnUpdateHunt = function(inst, hunt)
        ...
        if hunt.lastdirttime ~= nil then
            if hunt.huntedbeast == nil and hunt.trackspawned >= 1 then
                ...
                if (wet and (GetTime() - hunt.lastdirttime) > (.75 * TUNING.SEG_TIME)) or
                    (GetTime() - hunt.lastdirttime) > (1.25 * TUNING.SEG_TIME) then
    ...
    local function SpawnHuntedBeast(hunt, pt, doer)
        ...
        local spawn_pt = GetSpawnPoint(pt, TUNING.HUNT_SPAWN_DIST, hunt)
        if spawn_pt ~= nil then
            hunt.huntedbeast = SpawnPrefab((self:IsWargShrineActive(doer) and "claywarg") or
    ...
    function self:IsWargShrineActive(doer)
        return _wargshrines:Get() and
                   (IsSpecialEventActive(SPECIAL_EVENTS.YOTV) or doer.components.allachivcoin.shrine == true)
    end
    ...
    function self:GetDebugString()
        ...
        for i, hunt in ipairs(_activehunts) do
            ...
            if hunt.trackspawned ~= nil then
                str = str .. " Track # " .. tostring(hunt.trackspawned) .. "/" .. tostring(hunt.numtrackstospawn) ..
