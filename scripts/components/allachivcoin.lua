local function getcoinamount(self, coinamount)
    self.inst.currentcoinamount:set(coinamount)
end

local function currenthungerup(self, hungerupamount)
    self.inst.currenthungerup:set(hungerupamount)
end
local function currentsanityup(self, sanityupamount)
    self.inst.currentsanityup:set(sanityupamount)
end
local function currenthealthup(self, healthupamount)
    self.inst.currenthealthup:set(healthupamount)
end
local function currenthealthregen(self, healthregenamount)
    self.inst.currenthealthregen:set(healthregenamount)
end
local function currentsanityregen(self, sanityregenamount)
    self.inst.currentsanityregen:set(sanityregenamount)
end
local function currenthungerrateup(self, hungerrateupamount)
    self.inst.currenthungerrateup:set(hungerrateupamount)
end
local function currentspeedup(self, speedupamount)
    self.inst.currentspeedup:set(speedupamount)
end
local function currentabsorbup(self, absorbupamount)
    self.inst.currentabsorbup:set(absorbupamount)
end
local function currentdamageup(self, damageupamount)
    self.inst.currentdamageup:set(damageupamount)
end
local function currentcrit(self, crit)
    self.inst.currentcrit:set(crit)
end
local function currentlifestealup(self, lifesteal)
    self.inst.currentlifestealup:set(lifesteal)
end
local function currentfireflylightup(self, fireflylightup)
    self.inst.currentfireflylightup:set(fireflylightup)
end
local function currentscaleup(self, scale)
    self.inst.currentscaleup:set(scale)
end

local function currenthungerachivcost(self, hungerachivcost)
    self.inst.currenthungerachivcost:set(hungerachivcost)
end
local function currentsanityachivcost(self, sanityachivcost)
    self.inst.currentsanityachivcost:set(sanityachivcost)
end
local function currenthealthachivcost(self, healthachivcost)
    self.inst.currenthealthachivcost:set(healthachivcost)
end
local function currenthealthregenachivcost(self, healthregenachivcost)
    self.inst.currenthealthregenachivcost:set(healthregenachivcost)
end
local function currentsanityregenachivcost(self, sanityregenachivcost)
    self.inst.currentsanityregenachivcost:set(sanityregenachivcost)
end
local function currenthungerrateachivcost(self, hungerrateachivcost)
    self.inst.currenthungerrateachivcost:set(hungerrateachivcost)
end
local function currentspeedachivcost(self, speedachivcost)
    self.inst.currentspeedachivcost:set(speedachivcost)
end
local function currentabsorbachivcost(self, absorbachivcost)
    self.inst.currentabsorbachivcost:set(absorbachivcost)
end
local function currentdamageachivcost(self, damageachivcost)
    self.inst.currentdamageachivcost:set(damageachivcost)
end
local function currentcritachivcost(self, critachivcost)
    self.inst.currentcritachivcost:set(critachivcost)
end
local function currentlifestealcost(self, lifestealcost)
    self.inst.currentlifestealcost:set(lifestealcost)
end
local function currentfireflylightcost(self, fireflylightcost)
    self.inst.currentfireflylightcost:set(fireflylightcost)
end
local function currentscalecost(self, scalecost)
    self.inst.currentscalecost:set(scalecost)
end

local function currentdoubledrop(self, doubledrop)
    local c = 0
    if doubledrop then
        c = 1
    end
    self.inst.currentdoubledrop:set(c)
end
local function currentkrampusxmas(self, krampusxmas)
    local c = 0
    if krampusxmas then
        c = 1
    end
    self.inst.currentkrampusxmas:set(c)
end
local function currentnomoist(self, nomoist)
    local c = 0
    if nomoist then
        c = 1
    end
    self.inst.currentnomoist:set(c)
end
local function currentgoodman(self, goodman)
    local c = 0
    if goodman then
        c = 1
    end
    self.inst.currentgoodman:set(c)
end
local function currentanimallover(self, animallover)
    local c = 0
    if animallover then
        c = 1
    end
    self.inst.currentanimallover:set(c)
end
local function currentnanobots(self, nanobots)
    local c = 0
    if nanobots then
        c = 1
    end
    self.inst.currentnanobots:set(c)
end
local function currentarchmage(self, archmage)
    local c = 0
    if archmage then
        c = 1
    end
    self.inst.currentarchmage:set(c)
end
local function currentcheatdeath(self, cheatdeath)
    local c = 0
    if cheatdeath then
        c = 1
    end
    self.inst.currentcheatdeath:set(c)
end
local function currentrefresh(self, refresh)
    local c = 0
    if refresh then
        c = 1
    end
    self.inst.currentrefresh:set(c)
end
local function currentfishmaster(self, fishmaster)
    local c = 0
    if fishmaster then
        c = 1
    end
    self.inst.currentfishmaster:set(c)
end
local function currentcookmaster(self, cookmaster)
    local c = 0
    if cookmaster then
        c = 1
    end
    self.inst.currentcookmaster:set(c)
end
local function currentchopmaster(self, chopmaster)
    local c = 0
    if chopmaster then
        c = 1
    end
    self.inst.currentchopmaster:set(c)
end
local function currentpickmaster(self, pickmaster)
    local c = 0
    if pickmaster then
        c = 1
    end
    self.inst.currentpickmaster:set(c)
end
local function currentbuildmaster(self, buildmaster)
    local c = 0
    if buildmaster then
        c = 1
    end
    self.inst.currentbuildmaster:set(c)
end
local function currenticebody(self, icebody)
    local c = 0
    if icebody then
        c = 1
    end
    self.inst.currenticebody:set(c)
end
local function currentfirebody(self, firebody)
    local c = 0
    if firebody then
        c = 1
    end
    self.inst.currentfirebody:set(c)
end
local function currentsupply(self, supply)
    local c = 0
    if supply then
        c = 1
    end
    self.inst.currentsupply:set(c)
end
local function currentengineering(self, engineering)
    local c = 0
    if engineering then
        c = 1
    end
    self.inst.currentengineering:set(c)
end
local function currentnaturalist(self, naturalist)
    local c = 0
    if naturalist then
        c = 1
    end
    self.inst.currentnaturalist:set(c)
end
local function currentlunarcraft(self, lunarcraft)
    local c = 0
    if lunarcraft then
        c = 1
    end
    self.inst.currentlunarcraft:set(c)
end
local function currentshrine(self, shrine)
    local c = 0
    if shrine then
        c = 1
    end
    self.inst.currentshrine:set(c)
end
local function currentreader(self, reader)
    local c = 0
    if reader then
        c = 1
    end
    self.inst.currentreader:set(c)
end
local function currentmasterchef(self, masterchef)
    local c = 0
    if masterchef then
        c = 1
    end
    self.inst.currentmasterchef:set(c)
end

local function currentminemaster(self, minemaster)
    local c = 0
    if minemaster then
        c = 1
    end
    self.inst.currentminemaster:set(c)
end
local function currentfastworker(self, fastworker)
    local c = 0
    if fastworker then
        c = 1
    end
    self.inst.currentfastworker:set(c)
end
local function currentancientstation(self, ancientstation)
    local c = 0
    if ancientstation then
        c = 1
    end
    self.inst.currentancientstation:set(c)
end
...
function allachivcoin:OnLoad(data)
    ...
    self.hungerachivcost = allachiv_coinuse["hungerup"] +
                               math.floor(self.hungerupamount / achievementcoststep["hungerup"])
    self.sanityachivcost = allachiv_coinuse["sanityup"] +
                               math.floor(self.sanityupamount / achievementcoststep["sanityup"])
    self.healthachivcost = allachiv_coinuse["healthup"] +
                               math.floor(self.healthupamount / achievementcoststep["healthup"])
    self.healthregenachivcost = allachiv_coinuse["healthregen"] +
                                    math.floor(self.healthregenamount / achievementcoststep["healthregen"])
    self.sanityregenachivcost = allachiv_coinuse["sanityregen"] +
                                    math.floor(self.sanityregenamount / achievementcoststep["sanityregen"])
    self.hungerrateachivcost = allachiv_coinuse["hungerrateup"] +
                                   math.floor(self.hungerrateupamount / achievementcoststep["hungerrateup"])
    self.speedachivcost =
        allachiv_coinuse["speedup"] + math.floor(self.speedupamount / achievementcoststep["speedup"]) * 2
    self.absorbachivcost = allachiv_coinuse["absorbup"] +
                               math.floor(self.absorbupamount / achievementcoststep["absorbup"]) * 2
    self.damageachivcost = allachiv_coinuse["damageup"] +
                               math.floor(self.damageupamount / achievementcoststep["damageup"]) * 2
    self.critachivcost = allachiv_coinuse["crit"] + math.floor(self.crit / achievementcoststep["crit"]) * 2
    self.lifestealcost = allachiv_coinuse["lifesteal"] +
                             math.floor(self.lifestealupamount / achievementcoststep["lifesteal"])
    self.fireflylightcost = allachiv_coinuse["fireflylightup"] +
                                math.floor(self.fireflylightup / achievementcoststep["fireflylightup"])
...
function allachivcoin:hungerrateupcoin(inst)
    if self.coinamount >= self.hungerrateachivcost and inst.components.hunger.hungerrate > .01 then
        ...
        inst.components.hunger.burnratemodifiers:SetModifier("achievementperk", 1 - self.hungerrateupamount *
            allachiv_coindata["hungerrateup"])
...
function allachivcoin:healthregenfn(inst)
    inst:DoPeriodicTask(5, function()
        if inst and inst.components.health and inst.components.health.currenthealth < inst.components.health.maxhealth and
            inst.components.health.currenthealth > 0 and self.healthregenamount > 0 then
...
function allachivcoin:sanityregenfn(inst)
    inst:DoPeriodicTask(5, function()
        if inst and inst.components.sanity and inst.components.sanity.current < inst.components.sanity.max and
            inst.components.health and inst.components.health.currenthealth > 0 and self.sanityregenamount > 0 then
...
function allachivcoin:speedupcoin(inst)
    if self.coinamount >= self.speedachivcost then
        ...
        self.speedachivcost = allachiv_coinuse["speedup"] +
                                  math.floor(self.speedupamount / achievementcoststep["speedup"]) * 2
        self:ongetcoin(inst)
    end
end
...
function allachivcoin:critfn(inst)
    inst:ListenForEvent("onhitother", function(inst, data)
        ...
        if target and math.random(1, 100) <= chance and not target:HasTag("wall") and self.crit > 0 and self.attackcheck ~=
            true and data.target.components.health and data.target.components.health.currenthealth > 0 and
            data.target.components.combat then
...
function allachivcoin:lifestealupcoin(inst)
    if self.coinamount >= self.lifestealcost then
        ...
        self.lifestealcost = allachiv_coinuse["lifesteal"] +
                                 math.floor(self.lifestealupamount / achievementcoststep["lifesteal"]) * 1
        self:ongetcoin(inst)
    end
end
...
local function IsValidVictim(victim)
    return victim ~= nil and
               not ((victim:HasTag("prey") and not victim:HasTag("hostile")) or victim:HasTag("veggie") or
                   victim:HasTag("structure") or victim:HasTag("wall") or victim:HasTag("balloon") or
                   victim:HasTag("groundspike") or victim:HasTag("smashable") or victim:HasTag("companion")) and
               victim.components.health ~= nil and victim.components.combat ~= nil
end

function allachivcoin:lifestealfn(inst)
    inst:ListenForEvent("onhitother", function(inst, data)
        if self.lifestealupamount > 0 and data.target and not inst.components.health:IsDead() and
            IsValidVictim(data.target) then
            ...
            inst.components.health:DoDelta(damage * allachiv_coindata["lifesteal"] * self.lifestealupamount, false,
                "lifesteal")
        end
    end)
end
...
function allachivcoin:fireflylightupcoin(inst)
    if self.coinamount >= self.fireflylightcost then
        ...
        self.fireflylightcost = allachiv_coinuse["fireflylightup"] +
                                    math.floor(self.fireflylightup / achievementcoststep["fireflylightup"]) * 5
        self:ongetcoin(inst)
...
SetSharedLootTable('krampusxmas',
    {{'monstermeat', 1.0}, {'pumpkincookie', 1.0}, {'krampus_sack', allachiv_coindata["krampusxmas"]}})
            ...
            for k, v in pairs(ents) do
                if v.prefab then
                    if v.prefab == "pigman" or v.prefab == "bunnyman" then
                        if v.components.follower.leader == nil and not v:HasTag("werepig") and not v:HasTag("guard") then
...
function allachivcoin:fishmastercoin(inst)
    if self.fishmaster ~= true and self.coinamount >= allachiv_coinuse["fishmaster"] then
        ...
        if inst.components.inventory:GetEquippedItem(EQUIPSLOTS.HANDS) and
            inst.components.inventory:GetEquippedItem(EQUIPSLOTS.HANDS).components and
            inst.components.inventory:GetEquippedItem(EQUIPSLOTS.HANDS).components.fishingrod then
    ...
    inst:ListenForEvent("minhealth", function(player, data)
        if self.cheatdeath and not onCoolDown and player.components.health.currenthealth <= 0 then
            ...
            player:DoTaskInTime(0.5, function(player)
                ...
                local innocent_tag = {"bird", "wall", "glommer", "butterfly", "berrythief", "rabbit", "mole",
...
function allachivcoin:minemasterfn(inst)
    if self.minemaster then
        inst:ListenForEvent("working", function(inst, data)
            if self.minemaster and data.target and
